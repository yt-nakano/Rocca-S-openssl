...
"AES-128-CBC-CTS", "CAMELLIA-128-CBC-CTS" and "Rocca-S" which were previously only
...
The following I<ctrl>s are supported for the Rocca-S AEAD algorithm.
...
This call can only be made when encrypting data and B<after> all data has been 
processed (e.g. after an EVP_EncryptFinal() call). the taglen must be 32.

=item EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, taglen, tag)

Sets the expected tag to I<taglen> bytes from I<tag>. 
This call is only legal when decrypting data and must be made B<before> any data is 
processed (e.g. before any EVP_DecryptUpdate() calls). the taglen must be 32.
